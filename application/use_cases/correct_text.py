...
class _LoraProxyPipeline:
    ...
    def correct(self, text: str, user_vocab: dict = None) -> list:
        # El adaptador LoRA ya NO recibe el texto crudo del alumno ni
        # reemplaza el pipeline: refina la salida de reglas+BETO+gramática
        # (igual que hace el T5 base en CorrectionPipeline._refine_with_model),
        # y su salida solo se acepta si pasa la misma guarda anti-alucinación
        # (is_safe_refinement). Así el adaptador solo tiene que aprender el
        # "acento" del usuario sobre una base ya limpia, no reconstruir toda
        # la corrección desde cero con muestreo estocástico.
        base_suggestions = self._base.correct(text, user_vocab)
        base_text = base_suggestions[0] if base_suggestions else text

        from infrastructure.ml.t5_model import generate_with_lora
        from infrastructure.ml.guards import is_safe_refinement
        try:
            candidates = generate_with_lora(
        ...
        for candidate in candidates:
            ...
            if is_safe_refinement(base_text, candidate):
                if base_text[:1].islower() and candidate[:1].isupper():
                    candidate = candidate[:1].lower() + candidate[1:]
                # refinado por LoRA primero, base como alternativa (mismo
                # patrón que _refine_with_model para el T5 base)
                return [candidate] + base_suggestions

        # Ningún candidato pasó la guarda: el adaptador está "alucinando"
        # para este texto puntual. Nos quedamos con el pipeline base.
        return base_suggestions
